...
\begin{itemize}
  ...
  \item $\mbox{二分图最大独立点集} = |V| – \mbox{二分图最小点覆盖集}$
  \item $\mbox{最大团} = \mbox{补图的最大点独立集}$
  \item $\mbox{有向图最小路径覆盖} = |V|– \mbox{最大匹配}$
  \item $\mbox{无向图最小路径覆盖} = |V|– \mbox{最大匹配}/2$
\end{itemize}
...
int bfs()
{
    ...
    while(!q.empty())
    {
        ...
        for(i=0; i<=n; i++)
        {
            ...
            {
                tflow[i]=tflow[t]<cap[t][i]?tflow[t]:cap[t][i];
                q.push(i);
                path[i]=t;
            }
        }
...
对于每一个点$i$，令
\begin{enumerate}
  \item $M_i$=$sum$($i$点所有流进来的下界流)$–$$sum$($i$点所有流出去的下界流)
  \item 新建源点S、汇点T
  \begin{itemize}
